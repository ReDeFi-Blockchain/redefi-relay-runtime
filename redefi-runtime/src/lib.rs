...
pub use fp_self_contained::SelfContainedCall;
...
use frame_support::{
	...
	parameter_types,
	traits::{
		fungible::HoldConsideration, tokens::UnityOrOuterConversion, ConstU32, ConstU8, Contains, EitherOf, EitherOfDiverse,
		FromContains,Get, InstanceFilter, KeyOwnerProofSystem, LinearStoragePrice, OnFinalize as _, PrivilegeCmp,
...
use pallet_evm::{account::CrossAccountId, Account as EVMAccount, FeeCalculator, Runner};
...
use pallet_transaction_payment::{FungibleAdapter, FeeDetails, RuntimeDispatchInfo};
use codec::{Decode, Encode, MaxEncodedLen};
/// Constant values used within the runtime.
use polkadot_runtime_constants::{currency::*, fee::*, time::*, TREASURY_PALLET_ID};
use polkadot_primitives::{
	slashing, AccountId, AccountIndex, ApprovalVotingParams, Balance, BlockNumber, CandidateEvent, CandidateHash,
	CommittedCandidateReceipt, CoreIndex, CoreState, DisputeState, ExecutorParams, GroupRotationInfo, Hash,
...
use polkadot_runtime_common::{
	...
	impls::{
		DealWithFees, ContainsParts, LocatableAssetConverter, VersionedLocatableAsset,
...
use xcm::{
	latest::{
		InteriorLocation, Junction, Junction::PalletInstance, Location,
	},
	VersionedLocation,
};
	...
	RuntimeCall: From<LocalCall>,
{
	fn create_transaction<C: frame_system::offchain::AppCrypto<Self::Public, Self::Signature>>(
		...
		let extra: SignedExtra = (
			...
			pallet_ethereum::FakeTransactionFinalizer::<Runtime>::decode(&mut [].as_slice())
				.unwrap(),
		);
...
parameter_types! {
	...
	pub LeaseOffset: BlockNumber = prod_or_fast!(0, 0, "DOT_LEASE_OFFSET");
}
...
mod benches {
	frame_benchmarking::define_benchmarks!(
		// Polkadot
		// NOTE: Make sure to prefix these with polkadot_`runtime_common::` so
...
sp_api::impl_runtime_apis! {
	...
	impl fp_rpc::EthereumRuntimeRPCApi<Block> for Runtime {
		...
		fn extrinsic_filter(xts: Vec<<Block as sp_runtime::traits::Block>::Extrinsic>) -> Vec<pallet_ethereum::Transaction> {
...
mod test_fees {
	...
	fn payout_weight_portion() {
		...
		let payout_weight =
			<Runtime as pallet_staking::Config>::WeightInfo::payout_stakers_alive_staked(max_nom_rewarded)
			.ref_time() as f64;
	...
	fn transfer_cost_min_multiplier() {
		...
		let uxt = UncheckedExtrinsic(sp_runtime::generic::UncheckedExtrinsic {
			function: call,
			signature: Some((
				MultiAddress::<sp_runtime::AccountId32, AccountIndex>::Id(Alice.to_account_id()),
...
mod remote_tests {
	...
	async fn remote_ext_test_setup() -> RemoteExternalities<Block> {
		let transport: Transport = var("WS")
			.unwrap_or("wss://rpc.polkadot.io:443".to_string())
			.into();
